"""Media download and processing utilities for WhatsApp and Instagram Cloud APIs."""
...
def download_from_url_and_store(
    url: str,
    bucket: str,
    prefix: str,
    file_id: str,
    ...
    """Download media from a direct URL (Instagram) and store in S3.

    Args:
        url: Direct HTTPS URL to the media file.
        bucket: S3 bucket name.
        prefix: S3 key prefix (e.g. "ig-images/").
        file_id: Unique ID for the file (used in S3 key).
        extension: File extension.

    Returns:
        Dict with s3_bucket, s3_key, and s3_url, or empty dict on failure.
    """
    if not url or not url.startswith("https://"):
        logger.error("Invalid or non-HTTPS URL: %s", url)
        return {}

    try:
        response = requests.get(url, timeout=60)
        if response.status_code != 200:
            logger.error("URL download failed: %s -> %s", url, response.status_code)
            return {}
        content = response.content
    except Exception as e:
        logger.error("Failed to download from URL %s: %s", url, str(e))
        return {}
